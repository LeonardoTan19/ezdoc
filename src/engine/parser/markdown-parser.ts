...
type ErrorHandler = (code: string, params?: Record<string, unknown>) => string

const defaultErrorHandler: ErrorHandler = (code, params) =>
  params?.details ? `${code}: ${String(params.details)}` : code
...
export class MarkdownParser {
  ...
  constructor(options: Partial<MarkdownOptions> = {}, errorHandler?: ErrorHandler) {
  ...
  parse(markdown: string, headingStyles?: Partial<Record<HeadingLevel, string | undefined>>): string {
    ...
    try {
      ...
      console.error(
        this.errorHandler('MARKDOWN_PARSE_ERROR', { details: String(error) }),
        error,
      )
      throw new Error(
        this.errorHandler('MARKDOWN_PARSE_FAILED', {
